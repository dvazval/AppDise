...
class CreateIngrediente(ModelForm):
	...
	self.fields['idingredienteTemp'] = forms.ChoiceField(label= 'Ingrediente', choices= [ (o, o.articulo  ) for o in Ingrediente.objects.all()])
	
    def save(self, commit=True, force_insert=False, force_update=False, *args, **kwargs):
        m = super(CreateIngrediente, self).save(commit=False, *args, **kwargs)
	m.idingrediente_id = Ingrediente.objects.get(pk=self.fields['idingredienteTemp'].get(value, u''))   #fields['idingredienteTemp'].choices )   
	m.idreceta_id = self.receta
	m.save()
